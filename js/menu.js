$(function () {
	$(".nav li").eq(0).mouseover(function () {
    $(".full_menu").eq(0).stop().slideDown(600);
  });
  $(".nav li").eq(1).mouseover(function () {
    $(".full_menu").eq(1).stop().slideDown(600);
  });
  $(".nav li").eq(2).mouseover(function () {
    $(".full_menu").eq(2).stop().slideDown(600);
  });
  $(".nav li").eq(3).mouseover(function () {
    $(".full_menu").eq(3).stop().slideDown(600);
  });
  $(".nav li").eq(4).mouseover(function () {
    $(".full_menu").eq(4).stop().slideDown(600);
  });
  $(".nav li").eq(5).mouseover(function () {
    $(".full_menu").eq(5).stop().slideDown(600);
  });
  $(".nav li").eq(6).mouseover(function () {
    $(".full_menu").eq(6).stop().slideDown(600);
  });












  $(".nav li,.full_menu_1").mouseleave(function () {
		$(".full_menu").stop().slideUp(600);

	});
});